...
- `TEMP_USER_ID = "00000000-0000-0000-0000-000000000001"` — MVP 하드코딩. Supabase Auth 연동 전까지 이 상수를 참고
- 백엔드는 `SUPABASE_SECRET_KEY`(서비스 롤)로 RLS 우회. 프론트는 `SUPABASE_ANON_KEY` 사용
- `core/supabase.py` — `.env` 로드 우선순위: `BriefUp/.env` → `backend/.env`
- `topic_curricula` — 커리큘럼 챕터 구조 + 검색 힌트를 JSONB로 저장하는 캐시 테이블
...
OPENAI_API_KEY        # GPT-4o-mini (요약, 퀴즈 생성)
ANTHROPIC_API_KEY     # Claude Haiku (오케스트레이션, 커리큘럼 생성, 검증)
TAVILY_API_KEY
FRONTEND_URL
```
...
- **실 사용자 인증**: Supabase Auth 미연동, `TEMP_USER_ID` 하드코딩이 의도적 결정
- **멀티 사용자**: 현재 단일 유저 기준 설계
